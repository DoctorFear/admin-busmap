...
export default function DriverAlertsPage() {
  ...
  const [alerts, setAlerts] = useState<string[]>([
    "Kẹt xe tại ngã tư Lê Lợi - 6:45 AM, 19/09/2025",
  ]);
  ...
  const fetchDriverTrips = async (id: number) => {
    try {
      ...
      if (data.success && data.data) {
        ...
        const activeTrip = data.data.find(
          (t: Trip) => t.status === "ONGOING" || t.status === "IN_PROGRESS"
        );
        if (activeTrip) {
  ...
  const handleAlertSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    ...
    try {
      ...
      const newAlert = `${alertMessage} - ${new Date().toLocaleString(
        "vi-VN"
      )} ✅`;
  ...
  const handleResolveAlert = async (alertID: number) => {
    ...
    try {
      const response = await fetch(
        `/api/driver-alerts/resolve/${alertID}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
