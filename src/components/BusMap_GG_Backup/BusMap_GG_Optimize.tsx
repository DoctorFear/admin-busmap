...
export default function BusMap_GG({ buses }: { buses: any[] }) {
          ...
          const stops: Stop[] = busStops
            .map((busStop, idx) => {
              ...
              return {
                ...
                lng,
                // RUN OKE
                name: `Điểm đón ${busStop.sequence}`, // Tên điểm dừng
                sequence: busStop.sequence, // Thứ tự trong tuyến (bắt đầu từ 1)                
  ...
  useEffect(() => {
    if (routes.length > 0 && selectedRouteIds.length === 0) {
      ...
      // const initial = routes.slice(1, NUMBER_TRACKING_ROUTES).map((r) => r.routeID);
      // const initial = [2 - 1, 4 - 1]; // Mặc định chọn tuyến 2 và 4
      const initial = [1];
  ...
  // ====================================================================
  // [4] TẠO ĐƯỜNG ĐI (POLYLINE) THEO ĐƯỜNG PHỐ
  // ====================================================================
