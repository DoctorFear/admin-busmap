...
export default function BusMap_GG({ 
  ...
  // - Nếu tuyến ĐƯỢC CHỌN (trong selectedRouteIds) → dùng màu thường và opacity 0.8
  // - Nếu tuyến KHÔNG ĐƯỢC CHỌN (bỏ chọn) → đổi opacity thành 0 (trong suốt hoàn toàn)
  // 
  // FLOW:
  // 1. Duyệt qua TẤT CẢ routes có path trong directionsPaths
  // 2. Kiểm tra route đó có được chọn không (trong selectedRouteIds)
  // 3. Nếu ĐƯỢC CHỌN → màu bình thường (đỏ, xanh, ...) + opacity 0.8
  //    Nếu BỎ CHỌN → opacity 0 (trong suốt)
  // 4. Render polyline với màu và opacity tương ứng
  //
  const polylines = useMemo(() => {
    ...
    const out = Object.keys(directionsPaths)
      .map((idStr) => {
        ...
        if (isSelected) {
          ...
          opacity = 0.8; // Hiển thị rõ ràng
        } else {
          // Tuyến bỏ chọn → trong suốt hoàn toàn
          color = "#999999"; // Màu không quan trọng vì opacity = 0
          opacity = 0; // Trong suốt hoàn toàn
        }
        
        console.log(`[5] ->_<- Tuyến ${id}: ${path.length} điểm, ${isSelected ? 'CHỌN' : 'BỎ CHỌN'} → màu ${color}, opacity ${opacity}`);
        ...
        return { 
          ...
          opacity,      // Độ trong suốt (0 = trong suốt, 0.8 = hiển thị)
    ...
    console.log("[5] ->_<- Đã tạo", out.length, "polylines:", 
      out.map(p => `ID=${p.id} ${p.isSelected ? 'CHỌN' : 'ẨN'}`));
    return out;
    ...
    <div style={{ position: "relative" }}>
      <GoogleMap mapContainerStyle={containerStyle} center={SGU_LAT_LNG} zoom={12}>
        ...
        */}
        ...
        - Nếu ĐƯỢC CHỌN → màu bình thường + opacity 0.8
        - Nếu BỎ CHỌN → opacity 0 (trong suốt hoàn toàn)
        
        CÁCH HOẠT ĐỘNG:
        1. Nhận polyline object từ useMemo [5]
        2. polyline.opacity đã được tính sẵn:
           - Tuyến được chọn → opacity 0.8 (hiển thị rõ)
           - Tuyến bỏ chọn → opacity 0 (trong suốt)
        3. Render Polyline với màu và opacity tương ứng
        4. TẤT CẢ polylines đều được render, nhưng bỏ chọn = trong suốt
        
        PROPS:
        - key: ID duy nhất của polyline
        - path: Array of {lat, lng} - đường đi từ Directions API
        - options.strokeColor: Màu đường
        - options.strokeOpacity: Độ trong suốt (0 = ẩn, 0.8 = hiển thị)
        - options.strokeWeight: Độ dày đường (5px)
        */}
        {(console.log("[7] ->_<- Render", polylines.length, "polylines:", 
          polylines.map(p => `ID=${p.id} opacity=${p.opacity}`)), null)}
        {polylines.map((pl) => {
          console.log(`[7.1] Render polyline tuyến ${pl.id}: màu = ${pl.color}, opacity = ${pl.opacity} (${pl.isSelected ? 'CHỌN' : 'ẨN'})`);
          return (
            <Polyline
              key={`polyline-${pl.id}`}
              path={pl.path}
