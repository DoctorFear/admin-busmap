...
export default function BusMap_GG({ 
  ...
  // ====================================================================
  // [1.5] TỰ ĐỘNG SELECT ROUTE ĐẦU TIÊN KHI ROUTES ĐƯỢC LOAD (CHO DRIVER PAGE)
  // ====================================================================
  // Chỉ chạy 1 lần khi routes được load xong
  // Không phụ thuộc vào buses prop để tránh bị override khi buses update vị trí
  useEffect(() => {
    // Chỉ auto-select nếu:
    // 1. Có routes
    // 2. Chưa có route nào được chọn
    // 3. Có buses prop (Driver mode)
    if (routes.length > 0 && selectedRouteIds.length === 0 && buses && buses.length > 0) {
      console.log('[1.5] ->_<- Auto-select route đầu tiên cho Driver');
      
      // Lấy routeID từ buses prop
      ...
      if (routeIDsFromBuses.length > 0) {
        // Chọn route đầu tiên
        const firstRoute = routeIDsFromBuses[0];
        setSelectedRouteIds([firstRoute]);
        console.log('[1.5] ->_<- Đã chọn route:', firstRoute);
      } else {
        // Fallback: Chọn route đầu tiên trong danh sách
        const firstRoute = routes[0].routeID;
        setSelectedRouteIds([firstRoute]);
        console.log('[1.5] ->_<- Fallback: Chọn route đầu tiên:', firstRoute);
      }
    }
  }, [routes]); // CHỈ PHỤ THUỘC VÀO ROUTES, KHÔNG PHỤ THUỘC VÀO BUSES
      ...
      <GoogleMap mapContainerStyle={containerStyle} center={SGU_LAT_LNG} zoom={12}>
        ...
        */}
        ...
        }
        {
          ...
          // CHỈ HIỂN THỊ BUS CỦA ROUTE ĐƯỢC CHỌN
          buses
            .filter((bus: any) => {
              // FILTER 1: Chỉ hiển thị bus của route được chọn
              const isBusRouteSelected = selectedRouteIds.includes(bus.routeID);
              if (!isBusRouteSelected) {
                console.log(`[6] -_- Bỏ qua bus ${bus.id} vì route ${bus.routeID} không được chọn`);
                return false;
              }
              
              // FILTER 2: VALIDATE lat/lng hợp lệ
            ...
            .map((bus: any) => {
              ...
              console.log(`[6] ->_<- Render bus ${bus.id} của route ${bus.routeID}`);
